...
This directory starts Phase 7. The data layer is now sufficient for a complete first manuscript draft; remaining COMSOL checks are manual validation rows that can be added to Section 4 after GUI runs.
...
| COMSOL validation | U/Vf0=0.6/CFFF passed; V/X, CFCF, and porous CSV-ready manual checks pending | `comsol/results/manual_validation_plan.csv` |
...
1. Write Methods first: Sections 2 and 3 are mostly stable and do not depend on pending COMSOL GUI work.
...
4. Fill Section 4 with the existing U/Vf0=0.6 validation, then append pending manual COMSOL rows after completion.

...
Length =0.2; %长m (若为柱状板壳单元，这里为直边长度，单位m)
Wide =0.5;  %宽m  (若为柱状板壳单元，这里为柱状板壳单元的弧度方向的弧度，单位 弧度)
MeshL= 10;  %长的网格个数
MeshW= 10;  %宽的网格个数
MaterialLayers= 12;  %有材料的层数
BoundaryConditions = 'FCFF';  %边界条件先后分别表示左、下、右、上的边界情况
Theory = 'FOSD';   %这里为用到的理论，填写“FOSD”和“Z”
LayersMax = 20; %最多可适用于划分材料的层数
Radius = 0.4; %若为柱状板则填写半径m

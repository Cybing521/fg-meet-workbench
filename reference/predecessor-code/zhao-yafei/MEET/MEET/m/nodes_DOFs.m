...
Length = 0.0508; %长m (Theta^1若为柱状板壳单元，这里为直边长度，单位m)
Wide = 3.14;  %宽m  (Theta^2若为柱状板壳单元，这里为柱状板壳单元的弧度方向的弧度，单位 弧度)
MeshL= 2;  %长的网格个数
MeshW= 10;  %宽的网格个数
MaterialLayers= 6;  %有材料的层数
BoundaryConditions = 'FFFC';  %边界条件为CFSH的排列，先后分别表示左、下、右、上的边界情况
Theory = 'F';   %这里为用到的理论，填写“F”、“T”、“Z”或“M”,F=FOSD,Z=Zigzag,T=TOSD,M=FOSD_HOSD_FOSD
LayersMax = 20; %最多可适用于划分材料的层数
Radius = 0.31831; %若为柱状板则填写半径m

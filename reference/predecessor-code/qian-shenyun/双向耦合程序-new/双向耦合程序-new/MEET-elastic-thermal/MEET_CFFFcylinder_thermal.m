...
% %% IMPORTANT SETINGS
% %% Initiation 
% PositionMx=98:150:1298;   %CFFF单边固支半圆柱壳，半径31.831cm，直边5.08cm，厚度为1.2mm，网格4X20，y=1.57m时中心线
% PositionMy=603:5:798;   %CFFF单边固支半圆柱壳，半径31.831cm，直边5.08cm，厚度为1.2mm，网格4X20，x=0.0254m时中心线
% PositionMEE =1:800;
% %%%%%
...
% Magnetic = 0;
% %划分10层  网格4X20 
%    PhiaMT(10:10:800) = -Voltmax;
%    PhiaMT(1:10:800) = Voltmax;
%    MgaT(10:10:800) = -Magnetic;
%    MgaT(1:10:800) = Magnetic; 
% %%%%%%%%%%
% FueT = FusT*Loadmax;
% FuaT = -KufMT*PhiaMT;
% FumT = -KuzT*MgaT;
...
%     Qd = CC(1:Tot_DOF_MEET);
%     SensM_T = CC(Tot_DOF_MEET+1:end);     
% X_Disp=0:0.003175:0.0254; 
% X_Disp1=0:0.0785:3.14; 
% %CFFF单边固支y=0.5m以及x=0.0254m的z向位移
...
% % 创建一个图形窗口并使用子图
% figure;
% % 绘制y=1.57m时沿x方向的中心线位移
% subplot(2, 1, 1);
% plot(X_Disp, Y1, '-ob', 'markersize', 5);
% ylabel('Centerline Displacement (mm)');
% xlabel('x-Distance');
% title('y=1.57m along X-axis z-Displacement');
% grid on;
% % 绘制x=0.0254m时沿x方向的中心线位移
% subplot(2, 1, 2);
% plot(X_Disp1, Y2, '-ob', 'markersize', 5);
% ylabel('Centerline Displacement (mm)');
% xlabel('y-Distance');
% title('x=0.0254m along Y-axis z-Displacement');
% grid on;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% %% 网格划分20X20X10
% %% IMPORTANT SETINGS
...
% PositionMx=153:225:13653;   %FFFC单边固支圆柱壳，半径600mm，直边300mm，厚度为6mm，网格30X30，y=0.15m时中心线
% PositionMy=6753:5:7048;   %FFFC单边固支圆柱壳，半径600mm，直边300mm，厚度为6mm，网格30X30，x=0.15m时中心线径向位移
PositionMEE =1:9000;
%%%%%
Qd = zeros(FinalDofM,1);
Qv = zeros(FinalDofM,1);
...
% Magnetic = 0;
% %划分11层  网格30X30 
%    PhiaMT(11:11:9900) = -Voltmax;
%    PhiaMT(1:11:9900) = Voltmax;
%    MgaT(11:11:9900) = -Magnetic;
%    MgaT(1:11:9900) = Magnetic; 
% %%%%%%%%%%
% FueT = FusT*Loadmax;
% FuaT = -KufMT*PhiaMT;
% FumT = -KuzT*MgaT;
...
% title('x=0.15m along Y-axis z-Displacement');
% grid on;
% SensTem_T = zeros(11,1);
% SensTem_T(1)=sum(SensM_T(1:11:9900))/900; %第1层输出电压平均值
% SensTem_T(2)=sum(SensM_T(2:11:9900))/900; %第3层输出电压平均值
% SensTem_T(3)=sum(SensM_T(3:11:9900))/900; %第3层输出电压平均值
% SensTem_T(4)=sum(SensM_T(4:11:9900))/900;%第4层输出电压平均值
% SensTem_T(5)=sum(SensM_T(5:11:9900))/900;%第5层输出电压平均值
% SensTem_T(6)=sum(SensM_T(6:11:9900))/900; %第6层输出电压平均值
% SensTem_T(7)=sum(SensM_T(7:11:9900))/900;%第7层输出电压平均值
% SensTem_T(8)=sum(SensM_T(8:11:9900))/900; %第8层输出电压平均值
% SensTem_T(9)=sum(SensM_T(9:11:9900))/900; %第9层输出电压平均值
% SensTem_T(10)=sum(SensM_T(10:11:9900))/900; %第10层输出电压平均值
% SensTem_T(10)=sum(SensM_T(11:11:9900))/900; %第11层输出电压平均值
% %% 网格划分15X15X20
% %% IMPORTANT SETINGS
% %% Initiation 

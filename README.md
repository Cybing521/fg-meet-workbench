...
输出目录为 `comsol/export/nonU/` 和 `comsol/export/porous/`。CFCF 代表工况也已导出 `comsol/export/Thermal_CFCF_X_Vf0.1-30x30-10layer_layers.csv`。最新 COMSOL batch 验证结果见 `reports/2026-06-01-comsol-validation/README.md`，目标清单和 MATLAB 参考值见 `comsol/results/manual_validation_plan.csv`。

当前 COMSOL batch 验证目标：

| 阶段 | 工况 | MATLAB 中心挠度 mm | 状态 |
|------|------|--------------------|------|
| 5.1 | V/Vf0=0.6/CFFF/elastic | -1.620839 | 15点最大误差 4.190%，通过 |
| 5.1 | X/Vf0=0.6/CFFF/elastic | -1.810358 | 15点最大误差 4.060%，通过 |
| 5.4 | X/Vf0=0.1/CFCF/elastic | -0.072521 | 15点最大误差 3.831%，通过 |
| 6.3 | U/Vf0=0.5/e0=0.2/Even/CFFF/elastic | -2.454739 | 中心误差 4.980%，15点最大误差 6.003% |
| 6.3 | U/Vf0=0.5/e0=0.3/Even/CFFF/elastic | -2.833957 | 中心误差 5.598%，15点最大误差 6.594% |
| 6.3 | X/Vf0=0.5/e0=0.2/Even/CFFF/elastic | -2.165514 | 中心误差 4.349%，15点最大误差 5.434% |

---

## 论文撰写工作区

阶段 7 已建立 `paper/` 目录，包含论文主稿骨架 `paper/main.tex`、数据清单与写作顺序 `paper/README.md`、图表归档目录 `paper/figures/` 和 `paper/tables/`。当前数据层已经足以启动 SCI 四区论文初稿；非 U 与 CFCF COMSOL 验证已通过，含孔隙 COMSOL 部分需要作为模型复核问题写入 Section 4。

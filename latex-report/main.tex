...
图~\ref{fig:fg-gradient-shell-schematic-report} 进一步说明材料分布梯度板壳的建模含义：板厚方向划分为 10 个材料层，每层按层中面体积分数换算为一组等效材料常数；MATLAB 与 COMSOL 均按这一逐层赋值逻辑体现梯度。右侧曲线给出 U/V/X/O/P 五种体积分数分布和三类孔隙退化模式，便于区分表面富集、中心富集与中面孔隙富集。

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{../paper/figures/fg_gradient_shell_schematic.png}
\caption{功能梯度材料分布与含孔隙板壳分层模型示意图}
\label{fig:fg-gradient-shell-schematic-report}
\end{figure}
...
\subsection{含孔隙FG-MEE静力扩展}

在版本二工作中，已完成含孔隙FG-MEE板的 $30\times30$ 静力参数扫描。参数空间包含 U/X 两种FG分布、5个体积分数、5个孔隙率水平和3种孔隙分布模式。由于 $e_0=0$ 时三种孔隙分布完全等价，仅保留 Even 基线，因此共生成130个输入算例；每个算例计算 Case A/B/C 三种载荷，最终得到390行结果，全部计算成功。
...
\noindent\textbf{主要发现：}
\begin{itemize}[leftmargin=2em]
\item 机械挠度对孔隙率最敏感。Case A中，U-Even-$e_0=0.4$ 的平均挠度放大最强，相对无孔隙基线为 1.742 倍；X-Even-$e_0=0.4$ 为 1.738 倍。
\item 非均匀中心富集孔隙（Uneven）对整体刚度削弱最弱；U-Uneven-$e_0=0.1$ 的平均挠度倍率仅为 1.031。
\item 层温差跨度与机械挠度呈相似放大趋势，Even-$e_0=0.4$ 的温差跨度倍率最高，说明孔隙导致刚度退化后，热--力耦合响应同步增强。
\item 磁电效率随孔隙率和分布模式非单调变化。当前平均效率最高组合为 U-Uneven-$e_0=0.1$，平均效率为 2.7678；最低组合为 X-LogUneven-$e_0=0.4$，平均效率为 1.3178。
\end{itemize}
...
\begin{subfigure}{0.48\textwidth}
\centering
\includegraphics[width=\textwidth]{../reports/2026-05-28-porous-static/figures/03_U_theta_ratio_vs_e0.png}
\caption{U分布温差倍率}
\end{subfigure}
\hfill
\begin{subfigure}{0.48\textwidth}
\centering
\includegraphics[width=\textwidth]{../reports/2026-05-28-porous-static/figures/03_X_theta_ratio_vs_e0.png}
\caption{X分布温差倍率}
\end{subfigure}
\caption{孔隙率对层温差跨度的影响}
\label{fig:porous-theta-ratio}
\end{figure}
...
\caption{X分布磁电效率}
\end{subfigure}
\caption{孔隙率对Case C磁电效率的影响}
\label{fig:porous-me-efficiency}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.78\textwidth]{../reports/2026-05-28-porous-static/figures/06_top_deflection_amplification_table.png}
\caption{孔隙导致的最大机械挠度放大组合}
\label{fig:porous-ranking}
\end{figure}

\subsection{多边界与含孔隙动力补充}

在最新补充实验中，已完成 CFCF 边界条件下的 $30\times30$ 静力扫描，以及含孔隙代表工况的 $10\times10$ 完整 Newmark 动力 pilot。

\begin{table}[H]
\centering
\caption{新增补充实验概览}
\label{tab:cfcf-porous-dynamic}
\begin{tabular}{p{4.0cm}p{4.0cm}p{5.0cm}}
\toprule
\textbf{实验} & \textbf{数据规模} & \textbf{关键结果} \\
\midrule
CFCF静力扫描 & U/X $\times$ 5个$V_{f0}$ $\times$ 3载荷，共30行 & 结果全部成功；elastic 中最强边界抑制为 X/$V_{f0}=0.1$，CFCF/CFFF 挠度比 0.055 \\
含孔隙动力代表 & U/$V_{f0}=0.5$/$e_0=0.2$/Even，$10\times10$，401步 & 静态中心挠度 $-2.4558$\,mm；动力峰值 $-5.2218$\,mm；超调比 2.126；一阶频率 54.62\,Hz \\
COMSOL分层导出 & 非U分布2组、含孔隙3组代表CSV & 已生成 \texttt{comsol/export/nonU/} 与 \texttt{comsol/export/porous/}，可直接供后续COMSOL验证调用 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{subfigure}{0.48\textwidth}
\centering
\includegraphics[width=\textwidth]{../reports/2026-06-01-cfcf-static/figures/04_X_cfcf_over_cfff_w_ratio.png}
\caption{X分布CFCF/CFFF挠度比}
\end{subfigure}
\hfill
\begin{subfigure}{0.48\textwidth}
\centering
\includegraphics[width=\textwidth]{../reports/2026-06-01-porous-dynamic/figures/02_porous_dynamic_center_timeseries.png}
\caption{含孔隙代表工况中心挠度时程}
\end{subfigure}
\caption{新增边界条件和含孔隙动力结果}
\label{fig:cfcf-porous-dynamic}
\end{figure}
...
含孔隙静力扫描 & \textcolor{passgreen}{完成} & 130算例/390行结果 \\
CFCF边界补充 & \textcolor{passgreen}{完成} & U/X共30行结果 \\
含孔隙动力pilot & \textcolor{passgreen}{完成} & 10$\times$10 Newmark 401步 \\
阻尼比敏感性 & \textcolor{passgreen}{完成} & 0/0.5/0.8/1.5\% \\
论文 \texttt{paper/main.tex} & \textcolor{passgreen}{完成} & 7章初稿+参考文献 \\
\midrule
非U分布COMSOL验证 & \textcolor{warnblue}{待推进} & CSV已就绪，GUI待跑 \\
含孔隙30$\times$30模态 & \textcolor{warnblue}{待推进} & 等$V_{f0}$频率退化 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{enumerate}
\item 选择 1--2 个含孔隙代表工况进行 COMSOL 分层验证，优先覆盖 Even-$e_0=0.4$ 和 Uneven-$e_0=0.2$；
\item 建立论文 \texttt{paper/main.tex} 目录，将含孔隙挠度、温差和磁电效率规律整理为正式论文图；
\item 将含孔隙动力从 $10\times10$ Newmark pilot 推进到 $30\times30$ 模态降阶，并比较孔隙引起的频率退化；

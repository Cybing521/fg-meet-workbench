...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Difine files
InputFile = './InputFile/Thermal_CFCFplate_0.6Vf-15x15-10layer.txt';
OutputFile = [];
...
CaseName = strcat('./CurrentComp/Thermal_CFCFplate_0.6Vf-15x15-10layer',StrTheory, IntegSchem);
...
% %% IMPORTANT SETINGS
% %% Initiation 
% PositionMx=53:155:5943;   %对边固支方板300mm*300mm*6mm，网格20X20，y=0.15m
...
% %% IMPORTANT SETINGS
% %% Initiation 
% PositionMx=78:230:13418;   %CFCF对边固支方板300mm*300mm*6mm，网格30X30，y=0.15m固定端沿自由端中心线
% % PositionMy=158:460:13038;   %CFCF对边固支方板300mm*300mm*6mm，网格30X30，y=0m固定端沿自由端中心线
% PositionMy=6598:5:6898;   %CFCF对边固支方板300mm*300mm*6mm，网格30X30，x=0.15m
% PositionMEE =1:9000;
% %%%%%
% Qd = zeros(FinalDofM,1);
% Qv = zeros(FinalDofM,1);
...
% X_Disp=0:0.005:0.3; 
% X_Disp1=0:0.005:0.3; 
% %CFCF单边固支y=0.15m以及y=0m的z向位移
% Y_DispLIN1=1000*Qd(PositionMx);
% Y_DispLIN2=1000*Qd(PositionMy);
%  Y1 = [0;Y_DispLIN1;0];
%  Y2 = [0;Y_DispLIN2;0];
% %CFCF单边固支y=0.15m以及x=0.15m的z向位移
% Y_DispLIN1=1000*Qd(PositionMx);
% Y_DispLIN2=1000*Qd(PositionMy);
%  Y1 = [0;Y_DispLIN1;0];
%  Y2 = [Y_DispLIN2];
...
%% IMPORTANT SETINGS
%% Initiation 
PositionMx=156:235:3211;   %CFCF对边固支方板，长宽均为300mm，厚度为6mm，网格15X15，y=0.15m时中心线
% PositionMx=158:235:3213;   %CFCF对边固支方板，长宽均为300mm，厚度为6mm，网格15X15，y=0.15m时中心线
PositionMy=1648:5:1723;   %CFCF对边固支方板，长宽均为300mm，厚度为6mm，网格15X15，x=0.15m时中心线径向位移
PositionMEE =1:2250;
%%%%%
Qd = zeros(FinalDofM,1);
Qv = zeros(FinalDofM,1);
...
%     CC=AA\BB;
%     Qd = CC(1:Tot_DOF_MEET);
%     SensM_T = CC(Tot_DOF_MEET+1:end)/(-0.0006); 
% %由热弹性本构方程计算温差
% FutT=zeros(2250,1);
%     Tot_DOF_MEET = length(KuuT(:,1));
%     AA=[KuuT,KutT; KtuT,KttT];

...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Difine files
InputFile = './InputFile/Thermal_CFCFcylinder_0.6Vf-30x30-10layer.txt';
OutputFile = [];
...
%% IMPORTANT SETINGS
%% Initiation 
PositionMx=148:220:13348;   %CFCF对边固支圆柱壳，半径600mm，直边300mm，厚度为6mm，网格30X30，直边方向中心线径向位移
% PositionMx=6752:5:7047;   %CFCF对边固支圆柱壳，半径600mm，直边300mm，厚度为6mm，网格30X30，曲边方向中心线环向位移
PositionMy=6603:5:6893;   %CFCF对边固支圆柱壳，半径600mm，直边300mm，厚度为6mm，网格30X30，曲边方向中心线径向位移
PositionMEE =1:9000;
%%%%%
Qd = zeros(FinalDofM,1);
Qv = zeros(FinalDofM,1);
...
grid on;
SensTem_total=sum(SensM_T(1:1:9000))/9000; %整体输出电压平均值
...
% %% IMPORTANT SETINGS
% %% Initiation 
% % PositionMx1=71:215:3296;   %CFCF对边固支方板，长宽均为300mm，厚度为6mm，网格15X15，y=0.15m时中心线
% % PositionMy1=1651:5:1716;   %CFCF对边固支方板，长宽均为300mm，厚度为6mm，网格15X15，x=0.15m时中心线径向位移
% % PositionMx2=72:215:3297;   %CFCF对边固支方板，长宽均为300mm，厚度为6mm，网格15X15，y=0.15m时中心线
% % PositionMy2=1652:5:1717;   %CFCF对边固支方板，长宽均为300mm，厚度为6mm，网格15X15，x=0.15m时中心线径向位移
% % PositionMx3=73:215:3298;   %CFCF对边固支方板，长宽均为300mm，厚度为6mm，网格15X15，y=0.15m时中心线
% % PositionMy3=1653:5:1718;   %CFCF对边固支方板，长宽均为300mm，厚度为6mm，网格15X15，x=0.15m时中心线径向位移
% PositionMx=73:215:3298;   %CFCF两直边固支圆柱壳，半径600mm，直边300mm，厚度为6mm，网格15X15，y=0.15m时中心线
% PositionMy=1653:5:1718;   %CFCF两直边固支圆柱壳，半径600mm，直边300mm，厚度为6mm，网格15X15，x=0.15m时中心线径向位移
% % PositionMx=158:235:3213;   %CFCF两曲边固支圆柱壳，半径600mm，直边300mm，厚度为6mm，网格15X15，y=0.15m时中心线
% % PositionMy=1648:5:1723;   %CFCF两曲边固支圆柱壳，半径600mm，直边300mm，厚度为6mm，网格15X15，x=0.15m时中心线径向位移
% % PositionMx=148:225:3298;   %CCFF两邻边固支圆柱壳，半径600mm，直边300mm，厚度为6mm，网格15X15，y=0.15m时中心线
% % PositionMy=1578:5:1648;   %CCFF两邻边固支圆柱壳，半径600mm，直边300mm，厚度为6mm，网格15X15，x=0.15m时中心线径向位移
% PositionMEE =1:2250;
% %%%%%
% Qd = zeros(FinalDofM,1);
% Qv = zeros(FinalDofM,1);
...
% X_Disp=0:0.02:0.3; 
% X_Disp1=0:0.02:0.3; 
% %CFCF对边固支y=0.15m以及x=0.15m的z向位移
% Y_DispLIN1=1000*Qd(PositionMx);
% Y_DispLIN2=1000*Qd(PositionMy);
%  Y1 = [Y_DispLIN1];
%  Y2 = [0;Y_DispLIN2;0];
% % %  %CFCF对边固支直边和曲边中心线三方向位移
% % Y_DispLINx1=1000*Qd(PositionMx1);
% % Y_DispLINx2=1000*Qd(PositionMx2);
% % Y_DispLINx3=1000*Qd(PositionMx3);
% % Y_DispLINy1=1000*Qd(PositionMy1);
% % Y_DispLINy2=1000*Qd(PositionMy2);
% % Y_DispLINy3=1000*Qd(PositionMy3);
% %  Yx1 = [Y_DispLINx1];
% %  Yx2 = [Y_DispLINx2];
% %  Yx3 = [Y_DispLINx3];
% %  Yy1 = [0;Y_DispLINy1;0];
...
% grid on;
% SensTem_T = zeros(10,1);
% SensTem_T(1)=sum(SensM_T(1:10:2250))/225; %第1层输出电压平均值
% SensTem_T(2)=sum(SensM_T(2:10:2250))/225; %第3层输出电压平均值
% SensTem_T(3)=sum(SensM_T(3:10:2250))/225; %第3层输出电压平均值
% SensTem_T(4)=sum(SensM_T(4:10:2250))/225;%第4层输出电压平均值
% SensTem_T(5)=sum(SensM_T(5:10:2250))/225;%第5层输出电压平均值
% SensTem_T(6)=sum(SensM_T(6:10:2250))/225; %第6层输出电压平均值
% SensTem_T(7)=sum(SensM_T(7:10:2250))/225;%第7层输出电压平均值
% SensTem_T(8)=sum(SensM_T(8:10:2250))/225; %第8层输出电压平均值
% SensTem_T(9)=sum(SensM_T(9:10:2250))/225; %第9层输出电压平均值
% SensTem_T(10)=sum(SensM_T(10:10:2250))/225; %第10层输出电压平均值
% %% 网格划分20X20X10
% %% IMPORTANT SETINGS
% %% Initiation 

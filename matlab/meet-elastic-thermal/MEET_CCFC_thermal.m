...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Difine files
InputFile = './InputFile/Thermal_CCFCplate_0.6Vf-30x30-10layer.txt';
OutputFile = [];
...
CaseName = strcat('./CurrentComp/Thermal_CCFCplate_0.6Vf-30x30-10layer',StrTheory, IntegSchem);
...
% %% IMPORTANT SETINGS
% %% Initiation 
% PositionMx=48:145:5703;   %CCFC三边固支方板300mm*300mm*6mm，网格20X20，y=0.15m固定端沿自由端中心线
% PositionMy=5608:5:5798;   %CCFC三边固支方板300mm*300mm*6mm，网格20X20，x=0.3m
...
% ylabel('Centerline Displacement (mm)');
% xlabel('y-Distance');
% title('x=0.3m along Y-axis z-Displacement');
% grid on;
% %% 网格划分20X20X20
% %% IMPORTANT SETINGS
% %% Initiation 
...
%% IMPORTANT SETINGS
%% Initiation 
PositionMx=73:220:13053;   %CCFC三边固支方板300mm*300mm*6mm，网格30X30，y=0.15m固定端沿自由端中心线
% PositionMy=158:460:13498;   %CCFC三边固支方板300mm*300mm*6mm，网格30X30，y=0m固定端沿自由端中心线
PositionMy=6308:5:6598;   %CCFC三边固支方板300mm*300mm*6mm，网格30X30，x=0.15m
PositionMEE =1:9000;
%%%%%
Qd = zeros(FinalDofM,1);
Qv = zeros(FinalDofM,1);
...
X_Disp=0:0.005:0.3; 
X_Disp1=0:0.01:0.3; 
% %CCFC三边固支y=0.15m以及y=0m的z向位移
% Y_DispLIN1=1000*Qd(PositionMx);
% Y_DispLIN2=1000*Qd(PositionMy);
%  Y1 = [0;Y_DispLIN1];
%  Y2 = [0;Y_DispLIN2];
 %CCFC三边固支y=0.15m以及x=0.15m的z向位移
Y_DispLIN1=1000*Qd(PositionMx);
Y_DispLIN2=1000*Qd(PositionMy);
 Y1 = [0;Y_DispLIN1];
 Y2 = [0;Y_DispLIN2;0];
...
subplot(2, 1, 2);
plot(X_Disp, Y2, '-ob', 'markersize', 5);
ylabel('Centerline Displacement (mm)');
xlabel('y-Distance');
